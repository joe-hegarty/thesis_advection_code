function u_new = limiter_clamp(u, a, dx, dt, u_1)
    %LIMITER_CLAMP If the new value is greater than or less than the values
    %              of the two interpolated sample points, clamp to within
    %              these two values
    ...
    for i = 1 : m;
        ...
        u_new(i+1) = min(u_new(i+1), u_max);
        u_new(i+1) = max(u_new(i+1), u_min);
    end
